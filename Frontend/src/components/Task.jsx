...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`${import.meta.env.VITE_BACKEND_URL}/project/${projectId}`)
                .then((res) => {
                    ...
                    setColumns({
                        ...
                        [uuid()]: {
                            name: "Doing",
                            items: res.data[0].task.filter((task) => task.stage === "Doing").sort((a, b) => {
                                return a.order - b.order;
                            })
                        },
                        [uuid()]: {
    ...
    const updateTodo = (data) => {
        axios.put(`${import.meta.env.VITE_BACKEND_URL}/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`${import.meta.env.VITE_BACKEND_URL}/project/${projectId}/task/${taskId}`)
            .then((res) => {

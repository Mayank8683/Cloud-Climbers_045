...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`${import.meta.env.VITE_BACKEND_URLL}/project/${id}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`${import.meta.env.VITE_BACKEND_URL}/project/`, { title, description: desc })
                .then((res) => {
                    closeModal()
            ...
            axios.put(`${import.meta.env.VITE_BACKEND_URL}/project/${id}`, { title, description: desc })
                .then((res) => {
                    closeModal()
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
